...
The AI Chat Assistant will be integrated into the existing React application, leveraging the Groq chat completions API (llama-3.1-8b-instant) for its conversational AI capabilities and the existing `menuData.js` for menu information.
...
-   **`groqService.ts`** (Existing - to be extended):
    ...
    -   This function will:
        -   Construct a detailed prompt for the Groq API, including the user's query, relevant conversation history (for context), and essential information about Little Lemon's menu (to guide responses and item selection).
...
4.  `groqService.ts` sends a prompt to the Groq API.
...
-   The Groq helper will use the `VITE_GROQ_API_KEY` from the `.env` file (legacy Gemini keys can be retired after migration).
...
2.  **Implement `sendMessageToGroq` in `groqService.ts` (Text-Based):**
    *   Develop an initial prompt tuned for the Groq-served Llama model focusing on answering questions about the Little Lemon menu and providing text-based recommendations.
